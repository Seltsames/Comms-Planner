-- ============================================================
-- Migration 00023: Analytics aggregates RPC (admin only)
-- ============================================================
-- Server-side aggregation for the AnalyticsView dashboard.
-- Avoids fetching 36k+ campaign_audience rows to the client.
-- Returns: KPIs, top 10 drivers, drivers/campaigns grouped by country & city,
--          per-campaign driver counts.
-- Filters applied server-side via p_country and p_channel.
-- Admin only.
-- ============================================================
...
BEGIN
  ...
  WITH
  -- Active campaigns (not rejected/cancelled), optionally filtered by country
  active_camps AS (
  ...
  -- Schedules for those campaigns, optionally filtered by channel
  ...
  -- Campaigns grouped by city: explode city_codes array
  camps_by_city AS (
    SELECT
  ...
  -- Per-driver totals (with channels + campaigns from filtered_schedules)
  driver_totals AS (
    SELECT
      ca.drv_id,
  ...
  -- Per-campaign driver list (just counts)
